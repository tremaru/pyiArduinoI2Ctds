...
bool    iarduino_I2C_TDS::setCalibration    (uint8_t num, uint16_t tds){                                                        //  Параметры:              num - стадия калибровки (1 или 2), tds - концентрация калибровочной жидкости от 0 до 10000 ppm.
            ...
            //  Готовим данные для записи:                                                                                      //
                data[0] = num==1? TDS_BIT_CALC_Kd_1:TDS_BIT_CALC_Kd_2;                                                          //  Устанавливаем бит запуска 1 или 2 стадии калибровки.
            //  Отправляем подготовленные данные в модуль:                                                                      //
...
bool    iarduino_I2C_TDS::setKnownTDS       (uint8_t num, uint16_t tds){                                                        //  Параметры:              num - стадия калибровки (1 или 2), tds - концентрация калибровочной жидкости от 0 до 10000 ppm.
            ...
            //  Отправляем подготовленные данные в модуль:                                                                      //
                ...
                return false;                                                                                                   //  Возвращаем ошибку.
                                                                                                                                ...
                                                                                                                                //
//      Получение общего сопротивления датчика:                                                                                 //  Возвращаемое значение:  сопротивление от 0 до 16'777,215 Ом.
float   iarduino_I2C_TDS::getRd             (void){                                                                             //  Параметр:               отсутствует.
            if(valAddr){                                                                                                        //  Если модуль был инициализирован, то ...
            //  Читаем данные из модуля:                                                                                        //
                _readBytes(REG_TDS_Rd, 3);                                                                                      //  Читаем 3 байта из регистра «REG_TDS_Rd» в массив «data».
            //  Возвращаем результат:                                                                                           //
                int32_t result = ((uint32_t)(data[2]&0x7F)<<16) | ((uint32_t)data[1]<<8) | data[0];                             //  Собираем результат из массива «data» в переменную «result».
                if( data[2]&0x80 ){ result*=-1; }                                                                               //  Добавляем отрицательный знак.
                return ((float)result)/1000.0f;                                                                                 //  Возвращаем результат, преобразовав значение из тысячных долей Ом.
            }                                                                                                                   //
            return 0;                                                                                                           //  Возвращаем ошибку.
}                                                                                                                               //
                                                                                                                                //
//      Установка общего сопротивления датчика:                                                                                 //  Возвращаемое значение:  результат установки.
bool    iarduino_I2C_TDS::setRd             (float val_Rd){                                                                     //  Параметр:               сопротивление от 0 до 16'777,215 Ом.
            if(valAddr){                                                                                                        //  Если модуль был инициализирован, то ...
            //  Проверяем полученные данные:                                                                                    //
                if( (val_Rd<0) || (val_Rd>16777.215f) ){ return false; }                                                        //
            //  Готовим данные для записи:                                                                                      //
                uint32_t result = (uint32_t)(val_Rd*1000.0f);                                                                   //  Преобразуем результат в тысячные доли.
                ...
                if( val_Rd<0 ){ data[2]|=0x80; }                                                                                //  Добавляем отрицательный знак.
            //  Отправляем подготовленные данные в модуль:                                                                      //
                if(_writeBytes(REG_TDS_Rd, 3)==false ){ return false; }                                                         //  Записываем 3 байта из массива «data» в регистр «REG_TDS_Rd».
                delay(50);                                                                                                      //  Даём время для сохранения данных в энергонезависимую память модуля.
                return true;                                                                                                    //  Возвращаем флаг успеха.
            }else{                                                                                                              //  Иначе, если модуль не инициализирован, то ...
                return false;                                                                                                   //  Возвращаем ошибку.
                                                                                                                                ...
                                                                                                                                //
//      Получение постоянной датчика:                                                                                           //  Возвращаемое значение:  постоянная датчика от 0 до 65,535 см^-1.
float   iarduino_I2C_TDS::getKd             (void){                                                                             //  Параметр:               отсутствует.
            if(valAddr){                                                                                                        //  Если модуль был инициализирован, то ...
            //  Читаем данные из модуля:                                                                                        //
                _readBytes(REG_TDS_Kd, 2);                                                                                      //  Читаем 2 байта из регистра «REG_TDS_Kd» в массив «data».
            //  Возвращаем результат:                                                                                           //
                uint16_t result = ((uint16_t)data[1]<<8) | data[0];                                                             //  Собираем результат из массива «data» в переменную «result».
                return ((float)result)/1000.0f;                                                                                 //  Возвращаем результат, преобразовав значение из тысячных долей см^-1.
            }                                                                                                                   //
            return 0;                                                                                                           //  Возвращаем ошибку.
}                                                                                                                               //
                                                                                                                                //
//      Установка постоянной датчика:                                                                                           //  Возвращаемое значение:  результат установки.
bool    iarduino_I2C_TDS::setKd             (float val_Kd){                                                                     //  Параметр:               постоянная датчика от 0 до 65,535 см^-1.
            if(valAddr){                                                                                                        //  Если модуль был инициализирован, то ...
            //  Проверяем полученные данные:                                                                                    //
                if( (val_Kd<0) || (val_Kd>65.535f) ){ return false; }                                                           //
            //  Готовим данные для записи:                                                                                      //
                uint16_t result = (uint16_t)(val_Kd*1000.0f);                                                                   //  Преобразуем результат в тысячные доли.
            ...
            //  Отправляем подготовленные данные в модуль:                                                                      //
                if(_writeBytes(REG_TDS_Kd, 2)==false ){ return false; }                                                         //  Записываем 2 байта из массива «data» в регистр «REG_TDS_Kd».
                delay(50);                                                                                                      //  Даём время для сохранения данных в энергонезависимую память модуля.
                return true;                                                                                                    //  Возвращаем флаг успеха.
            }else{                                                                                                              //  Иначе, если модуль не инициализирован, то ...
                return false;                                                                                                   //  Возвращаем ошибку.
...
bool    iarduino_I2C_TDS::set_T             (float val_T){                                                                      //  Параметр:               опорная температура от 0 до 63,75 °С.
            ...
            //  Отправляем подготовленные данные в модуль:                                                                      //
                ...
                return false;                                                                                                   //  Возвращаем ошибку.
                                                                                                                                ...
                                                                                                                                //
//      Получение общего измеренного сопротивления:                                                                             //  Возвращаемое значение:  сопротивление от Rd Ом до 10'000'000 Ом.
